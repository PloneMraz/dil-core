...
- **Tag schema, open layer.** The agent MAY mint further tags denoting what the datum is, such as format, platform, domain, or object, but never its quality, correctness, or value. Open tags MAY be added or removed; they never overwrite the fixed layer.
...
| F | The experience store: representation; index; selective-write versus store-all; private versus shared | An operational layer |
...
6. **Store (§9).** The ResistEvent is the atomic unit; the four fixed tags are present and never overwritten; host data entered only via the tagging-gate; the `[event]` log is append-only and no record in it was altered or removed; each scar carries its context anchor.
