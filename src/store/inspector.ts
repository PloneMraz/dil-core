...
export function inspectEventLog(log: EventLog): string {
  ...
  const lines = records.map((r, i) => {
    ...
    switch (r.activityKind) {
      ...
      case "revision":
        return `  #${i}  [revision] ${r.datumId} @T${r.issuingLayer} (c${r.cycleMark}) ${clock(r.t)}`;
      ...
      case "resistance-reading":
        return `  #${i}  [resistance-reading] ${r.source}/${r.entity} ${r.mismatchKind} rec=${r.recurrence} err=${r.delta}${r.signed} (c${r.cycleMark}) ${clock(r.t)}`;
    }
  });
