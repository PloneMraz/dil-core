...
test("a pre-versioning stored tag-schema is refused (policy B): its [event] chain cannot be verified", () => {
  ...
  fs.writeFileSync(layout.claimFile, JSON.stringify({ protocol: "0.3.3", tagSchema: 1, layout: 1 }));
...
test("a newer stored tag-schema is refused — an older DIL must not write it", () => {
  const root = tmpRoot();
  const layout = claimSubstrate(root);
  fs.writeFileSync(layout.claimFile, JSON.stringify({ protocol: "0.3.3", tagSchema: 999, layout: 1 }));
  assert.throws(() => claimSubstrate(root), SubstrateClaimError);
  fs.rmSync(root, { recursive: true, force: true });
});
