...
**Empty.** The last item — the §9 full-system commit/snapshot/recovery — shipped: git-style content-addressed markers in `store/commits/`, scar-rhythm cadence, whole-system restore, fork markers on rollback. Everything still open is open *by design*, below.
