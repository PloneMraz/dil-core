...
| **Contract** | **Content** |
| --- | --- |
| Input | Signal[] attesting that an activity-environment is present — drawn from the host's own existing data (the region in which operation takes place); what the region returns enters the store at `running`; requires no prior action |
...
- **R4 — Feedback into the loop.** The store is retrieved back into the running loop (INV-1), not a one-way read sink. What is written passes through the agency-gate (T2, SELF_WRITTEN) — the agent writes, not a third party on its behalf. A datum the agent writes anew enters the store at `nascent`; a revision of a datum it already holds changes that datum's content, not its position (below).

**Provenance tagging (position only, never truth-value).** Because the host may carry data that predates the loop, because the agent writes data of its own, and because a datum in use moves, each stored item is tagged purely by *where it presently stands*, never by whether it is right:
...
- **nascent** — data the agent has written anew while the loop runs: not something the host carried in, not something the region returned, and not a revision of a datum the agent already holds. It bears the cycle-mark of the cycle that wrote it. A one-way entry, like `prior`: once it has run, a datum never returns to it. The position names where the datum came from; it does not make the datum a part of the self, which is a law and not a stored core (§4).

- **running** — data in use, in motion through the loop; bears a cycle-mark (cycle-id / timestamp). What the region returns while the loop runs, and the loop's own per-cycle datum, enter here directly: they are in use the moment they arrive, and which class of data each is, is read from its `domain` tag, not from its position.
...
**Revision is not entry.** When the agent rewrites the content of a datum it already holds, the datum remains the same datum: its position does not change, and nothing enters. The revision is a change the agent wrote, read at T2 the next cycle as SELF_WRITTEN (INV-6), and it spawns one [event]: a bare record that datum D was revised at cycle N by layer L, carrying no content, as no [event] does. What D held before is not lost: it is in the snapshot of the commit that preceded the revision (below), and the commit marker ties that snapshot to its place in [event]. How finely successive revisions are captured is therefore the commit cadence, which is DECIDE@IMPL.
...
Two things follow, and both keep the store honest. First, INV-5 is untouched: what cycle-0 lacks is not *history* (which INV-5 forbids pre-loading) but a host-emitted *bootstrap action* — emitting a fresh action is not loading old memory. Second — and this is the responsibility boundary — DIL plus host only **store and tag position** (prior/nascent/running/simulated/projected/scar); the tag is mechanical (which of the six a datum currently occupies) and explicitly **carries no claim of correctness**. DIL has no faculty for judging true/false (not even partially). Judging correctness is the work of Mode-B and an external third party reading the scar. A prior bias that has not yet met resistance is therefore **not “wrong” — only “not yet tested”** (mistaking “not yet” for “not” is its own error); it is washed to tested status only when it collides and holds (becoming a scar), never merely by having run through cycles.
